...
const Weekly = () => {
  ...
  const addTotalToDatabase = async () => {
    setTotal(null);
    await setDoc(doc(db, "users", userId, "goalFortheWeek", "goal"), {
      goal: total,
      id: userId,
  ...
  useEffect(
    () =>
      onSnapshot(
        query(
          collection(db, "users", userId, "goalFortheWeek"),
      ...
      <ScrollView>
        ...
        <View style={tw`flex flex-col justify-center items-center`}>
          <Text style={[tw`mt-3 text-lg mb-2`, { color: colors.text }]}>
            Enter your weekly Goal to add Expenses
          </Text>
        ...
        <TouchableOpacity
          ...
          onPress={async () => {
            ...
            for (const ele of arrOfIds) {
              await deleteDoc(doc(db, "users", userId, "goalFortheWeek", ele));
            }
            ...
            {Housing.map((aim) => (
              ...
              >
                ...
                <TouchableOpacity
                  ...
                  onPress={async () => {
                    await deleteDoc(
                      doc(db, "users", userId, "goalFortheWeek", aim.id)
                    );
                  }}
                >
